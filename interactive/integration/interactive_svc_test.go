...
import (
	...
	"github.com/rwpp/RzWeLook/interactive/integration/startup"
...
func (s *InteractiveTestSuite) SetupSuite() {
	...
	//s.server = startup.InitInteractiveService()
}
...
func (s *InteractiveTestSuite) TestGetByIds() {
	...
	testCases := []struct {
		...
		wantErr error
		wantRes *intrv1.GetByIdResponse
	}{
		{
			name: "查找成功",
			biz:  "test",
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			res, err := svc.GetByIds(context.Background(), &intrv1.GetByIdRequest{
